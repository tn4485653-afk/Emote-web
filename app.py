...
async def SEndPacKeT(OnLinE , ChaT , TypE , PacKeT):
    ...
    if TypE == 'ChaT' and ChaT:
        ...
        if IN_TEAM and CURRENT_TEAM:
            ...
            EM = await GenJoinSquadsPacket(CURRENT_TEAM, key, iv)
...
async def perform_emote(team_code: str, uids: list, emote_id: int, bundle_name=None):
    global key, iv, region, online_writer, IN_TEAM, CURRENT_TEAM

    if online_writer is None:
        raise Exception("Bot not connected")

    try:
        # Join team only once and stay inside
        if CURRENT_TEAM != team_code:
        ...
        # ACTIVATE BUNDLE
        if bundle_name:
            bundle_id = bundle_map.get(bundle_name.lower())

            if bundle_id:
                try:
                    B = await bundle_packet_async(bundle_id, key, iv, region)
                    await SEndPacKeT(None, online_writer, 'OnLine', B)

                    print(f"Activated bundle: {bundle_name}")
        ...
        # Trigger emotes without leaving team
        for uid_str in uids:
            uid = int(uid_str)
...
def join_team():
    ...
    asyncio.run_coroutine_threadsafe(
        perform_emote(team_code, uids, emote_id, bundle_name), loop
    )
...
async def MaiiiinE():
    ...
    # BOT LOGIN UID
    BOT_UID = int('15508719940') 

    Uid, Pw = '4748380599', 'u6t5667BYSTARGMR5ao1NXzr'
    ...
    os.system('clear')
    print(render('M4 status exe', colors=['white', 'green'], align='center'))
